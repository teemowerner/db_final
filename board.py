...
from query import generation_movie, movie_company
...
st.markdown('''
            <p class="big-font">김하연, 김동건, 조영휘! 👋</p>
...
if select_tab == '제작사별':
    ...
    """
    # 🎬 제작사 이름을 입력해 주세요 ! 🕺💃🏻
    ## 예시 ) Warner Bros. , Paramount Pictures, Walt Disney Pictures
    """
    )
    title = st.text_input("제작사 이름", "Warner Bros.")
    # for i in range(10):
    #     if movie_company(title)[i]:
    length = len(movie_company(title))
    for i in range(length):
        st.write(movie_company(title)[i])
